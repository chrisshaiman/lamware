...
def _has_clr_header(file_path: Path) -> bool:
    ...
    try:
        ...
        if magic == 0x10b:  # PE32
            clr_offset = pe_offset + 24 + 208
        elif magic == 0x20b:  # PE32+
            clr_offset = pe_offset + 24 + 224
        else:
            return False
        if clr_offset + 8 > len(data):
            return False
        clr_rva = struct.unpack_from("<I", data, clr_offset)[0]
        return clr_rva > 0
...
def _find_embedded_dotnet(file_path: Path) -> Path | None:
    ...
    Returns path to carved PE in /tmp, or None if not found.
    """
    try:
        with open(file_path, "rb") as f:
            data = f.read()
    ...
    while offset < len(data) - 64:
        ...
        if pos == -1:
            break
        # Check if this MZ leads to a valid PE with CLR header
        chunk = data[pos:]
        if len(chunk) < 512:
            offset = pos + 2
            continue
        try:
            ...
            if clr_off + 8 > len(chunk):
                offset = pos + 2
                continue
            clr_rva = struct.unpack_from("<I", chunk, clr_off)[0]
            if clr_rva > 0:
                # Found embedded .NET PE — carve it out
                carved_path = Path(f"/tmp/carved_dotnet_{file_path.name[:16]}.exe")
...
def find_dotnet_extractions(cape_data: dict, cape_task_id: int | str | None,
    ...
    if cape_task_id:
        base_dir = Path(f"/opt/CAPEv2/storage/analyses/{cape_task_id}")
    ...
    for subdir_name, subdir in scan_dirs:
    ...
    if not results:
        for subdir_name, subdir in scan_dirs:
            ...
            large_files = sorted(
                [e for e in subdir.iterdir() if e.is_file() and e.stat().st_size > 51200],
                key=lambda e: e.stat().st_size,
                reverse=True,
            )
            for entry in large_files[:10]:  # cap scan to 10 largest
