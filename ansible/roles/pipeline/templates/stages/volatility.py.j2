...
def run_single_plugin(dump_path: Path, plugin: str, output_dir: Path,
    ...
    elif short_name == "malfind":
        plugin_timeout = 120   # 2 min cap — partial results acceptable, revisit later
    elif short_name in ("vadinfo", "handles"):
...
def run_volatility(cape_data: dict, output_dir: Path,
    ...
    Plugins are split into two phases:
      Phase 1 (parallel): independent read-only plugins that can run concurrently
      Phase 2 (sequential): malfind post-processing that depends on Phase 1 results
    """
    vol_start = time.time()
...
def extract_volatility_insights(plugins: dict) -> dict:
    ...
    if isinstance(handles, list):
        ...
        for handle in handles:
            ...
            if handle_type in ("Mutant", "Mutex"):
                ...
                if name.startswith(("\\BaseNamedObjects\\SM0:", "\\BaseNamedObjects\\WilStaging",
                    ...
                    continue
                if name not in mutex_holders:
                    mutex_holders[name] = []
                mutex_holders[name].append({"pid": pid, "process": process})
        ...
        if mutex_holders:
            # Sort by holder count descending — mutexes shared across many
            # processes are the most interesting (coordination/singleton checks)
            mutexes = []
            for name, holders in sorted(mutex_holders.items(),
                ...
                mutexes.append({
                    ...
                    # Keep pid/process of first holder for backward compatibility
                    "pid": holders[0]["pid"],
                    "process": holders[0]["process"],
                })
            insights["mutexes"] = mutexes[:50]
            insights["mutex_summary"] = {
                "unique_names": len(mutex_holders),
                "total_handles": sum(len(h) for h in mutex_holders.values()),
                "most_shared": mutexes[0]["mutex"] if mutexes else None,
                "most_shared_count": mutexes[0]["holder_count"] if mutexes else 0,
            }
        if suspicious_files:
