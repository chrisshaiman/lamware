# =============================================================================
# packer/windows11-base.pkr.hcl
# Windows 11 base image — builder tier for layered images.
#
# This image is NOT deployed directly. It serves as the base for:
#   - windows11-base.pkr.hcl (cleanup only → "clean" Cape snapshot)
#   - windows11-office.pkr.hcl (+ LibreOffice → cleanup → "office" Cape snapshot)
#
# What this builds:
...
#   9. WinRM and Administrator left ENABLED for layered builds
#  10. First-boot login completed — layered images boot straight to desktop
#
...
#   make win11-image
#   # or directly: cd packer/ && packer build -var-file=packer.auto.pkrvars.hcl windows11-base.pkr.hcl
#
# After build:
#   Copy packer/output/windows11-base.qcow2 to the bare metal host.
...
#     packer build -var headless=false -var-file=packer.auto.pkrvars.hcl windows11-base.pkr.hcl
...
source "qemu" "windows11_base" {
  ...
  vm_name          = "windows11-base.qcow2"
  # This is the builder base — NOT a production image
  format           = "qcow2"
  ...
  # Graceful shutdown — keep Administrator and WinRM enabled for layered builds.
  # Cleanup (WinRM removal, Administrator disable) happens in the derived images
  # (windows11-guest.pkr.hcl, windows11-office.pkr.hcl).
...
build {
  name    = "windows11-base"
  sources = ["source.qemu.windows11_base"]
