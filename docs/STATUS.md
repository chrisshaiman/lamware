...
| Cross-copy drift guard for the agent arg validators | MED (top open) | `api/app/investigate/tool_validators.py` `GHIDRA_ARG_VALIDATORS` is a **manual mirror** of the pipeline's `TOOL_ARG_VALIDATORS` (`ansible/roles/pipeline/templates/stages/interpret.py.j2`). #114 added only an *intra-file* guard (`set(GHIDRA_ARG_VALIDATORS) == _GHIDRA_TOOLS`); nothing checks the agent copy against the pipeline **source of truth**. If the pipeline tightens a regex/bound or adds a tool, the agent path silently keeps the looser rule → a quiet bypass of the validation control just built. Fix: a CI test that extracts `TOOL_ARG_VALIDATORS` from the `.j2` (render/parse the validator block) and asserts it equals the agent dict — fail on any divergence. Same drift-test pattern already used for Alembic ORM↔DB. **Ranked above the ruff/timeout hygiene items**: this is the difference between the validator staying trustworthy and becoming a future bypass. Permanent fix = de-template the pipeline so both import one shared module (see de-template backlog). |
