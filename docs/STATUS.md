...
| Item | Effort | Notes |
|------|--------|-------|
| De-template run-pipeline.py.j2 + decompose orchestrator | Phase 1 DONE; Phases 2-4 remain | **Phase 1 landed (feature/detemplate-pipeline-phase1):** new `shared/` (`lamware_shared`) + `pipeline/` (`lamware_pipeline`) packages; the dual Ghidra validator collapsed to ONE shared module imported by both api and the interpret `.j2` (drift guard deleted — fork killed at the root); the four pure helpers (`cross_correlate`/`determine_family`/`calculate_severity`/`build_mitre_mapping`) extracted to `lamware_pipeline.correlation` with characterization tests (behavior verified identical to source); `PipelineConfig` pydantic + `config.json` proven on the malfind slice (the config-injection POC); both packages now under CI (ruff/semgrep/pytest) + Ansible pip-installs them system-wide (api venv too). **Phase 2:** migrate the `run_pipeline` body (~1184 lines) + the rest of the `99-154` constant block to `PipelineConfig` (full ~45 vars). **Also give the pipeline its own venv** (like the api role) instead of `pip install --break-system-packages` into system Python — PR #123 had to shim around an apt-managed `typing_extensions` that pip couldn't upgrade for pydantic; this conflict class recurs as the pipeline gains deps. A dedicated venv (update `run-pipeline-wrapper.sh.j2` + the role's pip tasks to target it) isolates from apt and removes the `--ignore-installed` shim. **Phase 3:** the 13 `stages/*.py.j2`. **Phase 4:** retire `.j2` deploy entirely. Incremental, behavior-preserving. |
