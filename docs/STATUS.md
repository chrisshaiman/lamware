...
| Alembic Spec 2 — complete ORM + autogenerate (fast-follow) | 1 session | The agreed follow-up to Alembic Phase A: add the ~6 missing table models (`tags`, `analysis_tags`, `ioc_tags`, `sample_tags`, `ioc_technique_mappings`, `pipeline_stage_events`, `audit_log`, `sample_relationships`) so `SQLModel.metadata` fully mirrors the DB, add an "autogenerate produces an empty diff" drift test (permanent drift sentinel), then flip `env.py` `target_metadata` from `None` to `SQLModel.metadata` to enable `--autogenerate`. Spec at `docs/superpowers/specs/2026-06-13-alembic-baseline-migrations-design.md` (local, gitignored). |
