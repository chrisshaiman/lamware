...
| Alembic Spec 2 — complete ORM + autogenerate (fast-follow) | ~half day | Add the **8** missing table models (`tags`, `analysis_tags`, `ioc_tags`, `sample_tags`, `ioc_technique_mappings`, `pipeline_stage_events`, `audit_log`, `sample_relationships`) so `SQLModel.metadata` fully mirrors the DB, add an "autogenerate produces an empty diff" drift test (permanent drift sentinel), then flip `env.py` `target_metadata` from `None` to `SQLModel.metadata` to enable `--autogenerate`. **Difficulty = low/mechanical, not design:** all 8 structures already exist verbatim in `0001_baseline.py` — transcribe them (3 are trivial 2-FK link tables, 5 are simple). ~1-2 hrs to write the classes; the real work is the self-checking reconciliation loop (run `--autogenerate`, it prints exactly what mismatches, fix, repeat until empty diff). Budget for known first-pass mismatches: `server_default` on `now()`/`false` columns, `jsonb` + the `varchar(100)[]` array col, and configuring autogenerate to IGNORE the 5 views (handled by raw-SQL migrations like 0002) + possibly the GIN trigram indexes. Good fit to hand off mechanically. Spec at `docs/superpowers/specs/2026-06-13-alembic-baseline-migrations-design.md` (local, gitignored). |
| Alembic 0002 — normalize infrastructure_overlap view | 1-2 hrs | First *real* Alembic migration: `CREATE OR REPLACE VIEW infrastructure_overlap` with the canonical array-cast form. Satisfies Phase B criterion #2 (a real change shipped end-to-end via a revision) AND makes the equivalence-diff reach a clean `[OK]` (today's sole diff is this view's semantically-identical pg_dump re-serialization). Touches prod, so do it deliberately. Unblocks Phase B retirement of the legacy `.sql` files. |
