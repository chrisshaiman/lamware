...
| 4.5 AI RE | Claude tool_use (agentic for native PE, single-shot for .NET/Go/Python/Java/VBA/PS) | Podman (--network=none, LiteLLM socket) | Complete |
| 4.7 Evasion hunter | Claude (single-shot) — triggers on low-activity samples | Podman (--network=none, LiteLLM socket) | Complete |
| 5.5 Screenshot analysis | Perceptual dedup + QR detection | Podman (--network=none) | Complete |
| 5.7 Visual analysis | Claude multimodal (screenshot interpretation) | Podman (--network=none, LiteLLM socket) | Complete |
| 5. Executive summary | Claude Haiku (single-shot) | Podman (--network=none, LiteLLM socket) | Complete |
| 5.1 Plain English | Claude Haiku (non-technical explanation) | Podman (--network=none, LiteLLM socket) | Complete |
...
| Item | Effort | Notes |
|------|--------|-------|
| De-template run-pipeline.py.j2 + decompose orchestrator | **DONE — all phases shipped + deployed 2026-06-25** | **COMPLETE.** Phase 1 (#122) · 2a venv (#125-127) · 2b-1/2b-2/2b-3 (#130/#131/#132) + stale-venv force-reinstall (#133) · 3a/3b/3c (#134/#135/#137) + SQL→psycopg2.sql composition (#136). `run-pipeline.py` + every helper/stage are now **plain importable Python**; the pipeline role has **zero `.py.j2`**; a real `--help` import smoke replaced the py_compile stand-in; deployed + live-validated (config loads through the strict `extra="forbid"` model, DB writes via the env password, `make smoke` 14/14). The agentic-orchestrator seam is open. Phase 4 (other-role container-entrypoint templates) is optional. _Historical roadmap below._ <br><br> **Phase 1 landed (feature/detemplate-pipeline-phase1):** new `shared/` (`lamware_shared`) + `pipeline/` (`lamware_pipeline`) packages; the dual Ghidra validator collapsed to ONE shared module imported by both api and the interpret `.j2` (drift guard deleted — fork killed at the root); the four pure helpers (`cross_correlate`/`determine_family`/`calculate_severity`/`build_mitre_mapping`) extracted to `lamware_pipeline.correlation` with characterization tests (behavior verified identical to source); `PipelineConfig` pydantic + `config.json` proven on the malfind slice (the config-injection POC); both packages now under CI (ruff/semgrep/pytest) + Ansible pip-installs them system-wide (api venv too). <br> **Phase 2a — Pipeline venv (NEXT-UP, FOUNDATIONAL — do before the code migration):** give the pipeline its own venv (like the api role) instead of `pip install --break-system-packages` into system Python. It is *orthogonal* to the code work (runtime env, not code structure), *de-risks every later phase* (each phase pulls more deps into the package; on system Python each is another apt-conflict like PR #123's `typing_extensions`-can't-uninstall shim), and is *small + behavior-preserving + independently shippable*. Scope: create the venv, install the packages + `requests`/`weasyprint`/`markdown` into it, point `run-pipeline-wrapper.sh.j2` at the venv python, retarget the role's pip tasks, drop the `--ignore-installed` shim. **Also clean up the api-venv install perms hack from PR #124** (the firefight fix for the post-Phase-1 502): replace the root-install + `chmod -R a+rX` **shell-with-glob** (hardcoded `python3.12` path, `changed_when: false`) with a **`become_user: lamware-api` install** + making the pipeline-**staged source group-traversable** (`/opt/pipeline/pkg/*` dirs `0750`, not just `0644` files). That removes the chmod, the shell, the glob, AND the version-coupled path in one move — files land owned+readable by the service user with no post-fix. NOTE: multiple venvs each installing `lamware_shared` is NOT drift (one source package, N runtime installs). It's its own spec→plan→PR. <br> **Phase 2b:** migrate the `run_pipeline` body (~1184 lines) + the rest of the `99-154` constant block to `PipelineConfig` (full ~45 vars) — opens the seam for [[project_agentic_orchestrator]]. <br> **Phase 3:** the 13 `stages/*.py.j2`. **Phase 4:** retire `.j2` deploy entirely. Incremental, behavior-preserving. |
...
| Cross-copy drift guard for the agent arg validators | RETIRED (root fix shipped) | Was `api/tests/test_validator_drift.py` (#118), guarding the two hand-synced validator copies. **De-template Phase 1 dissolved the fork at the root:** there is now ONE `lamware_shared.tool_validators` imported by both the api and the interpret `.j2`, so nothing can drift and the guard was deleted. The interim guard did its job; the permanent fix replaced it. |
| Prompt-injection regression test | DONE | **DONE — `test_injection_args_rejected_before_dispatch` in `test_investigate_tools.py`.** Parametrized battery of injection-style tool args (shell metachars, `$(...)`, path traversal, newline/instruction, over-bound range/length) driven through `execute_tool` with a spy asserting an error AND zero dispatch to `_ghidra_tool`. Complements the input-side delimiter-escape test in `test_system_prompt.py` (#111-era) and #114's arg-shape rejection. Note documented: the free-form `name` arg (`^.{1,200}$`) is intentionally permissive — safety is JSON-not-shell argv + Ghidra null-on-unknown-symbol, not the regex — so `name` metachars are inert and not asserted. |
| Source LLM pricing from LiteLLM, not hardcoded dicts | MED (deferred — batch with LiteLLM spend tracking) | `orchestrator.py` `MODEL_COSTS` (and `_calculate_llm_cost` in db_ingest) hardcode per-token prices that drift on model updates; LiteLLM already tracks spend — read cost from its usage/spend response. Ties to "Separate manual-analysis cost category." **Latent, not broken (verified 2026-06-21):** `MODEL_COSTS` keys exactly match `VALID_MODELS` (`investigate.py`: sonnet-4-6/opus-4-6/haiku-4-5), so cost is correct today. Risk is the silent fallback — `MODEL_COSTS.get(model, {0.0,0.0})` costs any unknown/bumped model at **$0** with no error; three hand-synced lists (VALID_MODELS + MODEL_COSTS + db_ingest table) + stale prices on Anthropic changes. |
| Extend ruff to api/ in CI | DONE | **DONE — `ci.yml` now runs `ruff check src/ api/`.** Cleared 63 pre-existing findings: 54 autofixed (33 `datetime.UTC`, import-sort, f-strings, unused imports), `feeder.py` E402 fixed by moving a misplaced `log =` below the imports, dead `call_failed` var removed from `orchestrator.py`, and a tests-only `E402` per-file-ignore added (exec-with-stubs / auth purge patterns; production stays strict). |
...
| run_python limit — single source of truth | DONE | **DONE — single-sourced from the `python-sandbox` Ansible role.** `lamware-api.env.j2` exports `python_sandbox_container_timeout`/`_container_memory`/`_max_script_size` as `LAMWARE_SANDBOX_*`; api config reads them (`sandbox_timeout_seconds`/`_memory_mb`/`_max_script_bytes`). `_run_python` uses them for the script-size check and an outer backstop (`timeout + 10`); the tool docstring is built from them so it can't drift. The wrapper's hardcoded "256MB/1CPU/30s" comment now interpolates the same vars. |
...
### Deeper architectural review (2026-06-21)

Second adversarial pass, this time on the pipeline interpret stage, the interpret-container wrapper, the broker loop, and the deploy surface (not the API hot paths). Each claim re-validated against code; verdicts noted. New actionable items:

| Item | Pri | Notes |
|------|-----|-------|
| Re-scope the interpret container off `--network=host` | **DONE — #138/#139 (2026-06-25), deployed + live-validated** | **DONE — shipped the tightest option: Unix socket + `--network=none`.** The interpret container now reaches LiteLLM only via a bind-mounted Unix socket (root `socat` bridge fronting `:4000`); `/run/litellm` is `0750 root:pipeline`; isolation is guarded at three layers (static CI, deploy stat-assert, runtime `netcheck` probe) and live-validated (`nobody` blocked from the socket; a real Claude call succeeds over it). The stale `--network=host` comments in `interpret.py`/`tasks/main.yml` were corrected and the vestigial `4000`/`27017` pipeline-uid firewall allows identified. _Original analysis below._ <br><br> [run-interpret-wrapper.sh.j2:16](../ansible/roles/interpret/templates/run-interpret-wrapper.sh.j2) runs the LLM-broker container with `--network=host` — the one component touching malware-derived LLM I/O, and the only one that breaks the `--network=none` pattern. **Severity nuance (validated):** NOT "max blast radius" — the same wrapper sets `--read-only --cap-drop=ALL --security-opt=no-new-privileges --user 65534:65534` + tmpfs-only, so code-exec is hard; the real residual is **network reachability** to host loopback services (Postgres/Keycloak/LiteLLM/API/Mongo) it has no need to reach. Fix (best first): put interpret + LiteLLM on a dedicated Podman network so interpret reaches only `:4000`; or run LiteLLM on a Unix socket bind-mounted into a `--network=none` container (tightest). Supersedes the inherited iptables-owner "LiteLLM network lockdown" plan (enforces at the wrong layer). |
| Split "full report" from "LLM context view" | MED | [interpret.py.j2:247-252](../ansible/roles/pipeline/templates/stages/interpret.py.j2) confirms reports hit **42MB+** (malfind/volatility) and are shipped to the summarize LLM call via `communicate()`. The merged report does double duty: analyst artifact AND LLM input. They want different shapes — the LLM wants a condensed, reference-bearing view, not raw malfind dumps. Splitting them cuts token cost and removes the 42MB-through-a-pipe smell. Ties to [[project_correlation_engine]] / token-cost work. |
| Decide: `check_prompt_influence` acts or gets relabeled | LOW | Validated: the flag is computed ([interpret.py.j2:166](../ansible/roles/pipeline/templates/stages/interpret.py.j2)), stored (db_ingest), and shown in the report — but the only runtime use is `log.info(... Influence flag: ...)` (run-pipeline.py.j2:1558). No alert/escalation/quarantine/re-run. Keyword list is 5 literals → trivially bypassed by a verdict-softening that avoids the words. Either wire it to an action (raise review priority / second model pass / ntfy) or relabel it as telemetry and stop calling it an injection mitigation. The genuinely-strong injection defense is structural (output-only LLM, no verdict authority). |

Also validated, lower priority / not acted on: broker `readline()` timeout is checked only at loop top so a silent container hang isn't caught at `interpret_timeout` ([interpret.py.j2:138-152](../ansible/roles/pipeline/templates/stages/interpret.py.j2)) — but it IS bounded by the container's podman `--timeout` (EOF → break), so low severity; the inter-container JSON protocol is hand-parsed with `.get()` and no version field (robustness nicety). **Dismissed as overstated:** "every change re-runs the whole play" — `site.yml` already has per-role tags (`--tags api`/`keycloak`/`pipeline`), so targeted deploys work today; a tier meta-tag (infra/platform/app) is at most a minor convenience. The review's #1 (templating hides the pipeline from CI) and #4 (validator fork) are accurate but already tracked — see [[project_detemplating_pipeline]] (HIGH; dissolves the fork at the root) and the **shipped** `test_validator_drift.py` drift guard above (the review's "you can't even write that guard" is outdated).
...
| MalwareBazaar sample catalogue (analyst pick-and-choose) | Design + build | Analyst-facing UI to **browse what's available on MalwareBazaar and select specific samples to detonate**, instead of only the automated feed. Backend: FastAPI endpoint querying the MB API (recent additions + by tag / signature(family) / file_type / imphash / yara / clamav) → normalized catalogue (sha256, filename, file_type, family, first_seen, tags, size, reporter). Frontend: filterable/searchable grid with a per-sample "submit to pipeline" action (download via MB API → spool → run-pipeline; surface the analysis_id). Reuses the existing auto-feeder/sample-feeder MB integration + the spool submit path. Guardrails: MB Auth-Key in vault (never client-side), per-submit auth + logging, samples land mgmt-side then feed the air-gapped detonation VLAN (never the reverse). Open Qs: live query vs cached index, pagination over MB's corpus, dedupe vs already-analyzed (ties into campaign graph). Idea: Chris, 2026-06-25. |
