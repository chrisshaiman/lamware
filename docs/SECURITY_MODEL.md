...
> [!WARNING]
> **The detonation network is fully air-gapped.** `virbr-det` has no route to the management interface (`management_interface`, `enp3s0f0` on the current host — **not** literally `eth0`) or `wg0`. The air-gap DROP rules sit at `FORWARD` positions 4–5, above every other chain, so they are enforced before anything can accept. INetSim simulates internet services for guest VMs. All admin access is through WireGuard VPN.

> **"Enforced before any ACCEPT" is a property of rule ORDER, not of a rule existing.** On 2026-09-19 the pipeline user's egress DROP was present, correctly written, and had **0 packets** while uid 997 opened TCP to `1.1.1.1:443`, `9.9.9.9:443` and `1.1.1.1:53` and reached `127.0.0.1:27017` and `:4000`. ufw's chains sat above it and terminated traversal first. Every check was green throughout, because every check asked whether the rule existed.
>
> ufw has since been removed entirely — see [ADR-020](DECISIONS.md#adr-020-one-firewall-mechanism--iptables-persistent-not-ufw). The host runs **iptables-persistent only**, and the pipeline allowlist is back in `OUTPUT` where nothing precedes it. It permits exactly two destinations — PostgreSQL `5432` and the CAPE API `8000`, both over `lo` — and denies everything else including DNS. `cape_api_url` is `http://10.200.0.1:8000`, the host's own WireGuard address, which Linux routes via `lo`; that is why an `-o lo` rule covers it.
>
> Verified by behaviour, not by rule presence: DROP counter climbing, the four denied destinations refusing, the two allowed ones connecting. `network-monitor` asserts the same way every five minutes — it **attempts a connection the allowlist must refuse**, because a rule that exists satisfied every structural check for the whole day it did nothing.

> **Inbound is NOT default-deny.** `/etc/iptables/rules.v4` carries `:INPUT ACCEPT`. Any service that listens is reachable by whatever can route to it — which is how CAPE's web UI was exposed on `10.200.0.1:8000` with no firewall rule at all, unnoticed until ufw's removal made it unreachable and someone looked. ufw provided this property; dropping ufw gave it up and nothing has replaced it. Treat every new listener as world-reachable until that changes.

> **Verify containment before first detonation.** `make security-test` asserts the three properties this host rests on, by attempting each: the pipeline user cannot reach the internet, it is held to its localhost allowlist (denied ports refuse *and* allowed ports still work), and the first matching `FORWARD` rule for `virbr-det` is a DROP in both address families on both escape paths. The air-gap check is structural — a host cannot originate a forwarded packet — so `network-monitor` does the continuous version every five minutes, reading counters and probing egress directly. Run both after any infrastructure change. A misconfigured `virbr-det` route or a hypervisor escape turns this into a live-malware box with network — treat the containment checks as mandatory, not optional.
