...
> [!WARNING]
> **The detonation network is fully air-gapped.** `virbr-det` has no route to the management interface (`management_interface`, `enp3s0f0` on the current host — **not** literally `eth0`) or `wg0`. The air-gap DROP rules sit at FORWARD positions 4–5, ahead of every ufw chain, so they are enforced before any ACCEPT. INetSim simulates internet services for guest VMs. All admin access is through WireGuard VPN.

> **"Enforced before any ACCEPT" is a property of rule ORDER, not of a rule existing.** It held for the air-gap and did not hold for the pipeline user's egress allowlist, which was appended to `OUTPUT` and therefore evaluated *after* ufw's chains. Measured on the live host 2026-09-19: the DROP-all had 0 packets while the pipeline user opened TCP to `1.1.1.1:443`, `9.9.9.9:443` and `1.1.1.1:53`, and reached `127.0.0.1:27017` and `:4000` — the two ports the role explicitly revokes. ufw accepts all loopback, then its own outgoing list (22, 53, 80, 123, 443, 853, 4460 to **any** destination, from konstruktoid's `ufw_outgoing_traffic` default), before anything appended to `OUTPUT` is reached.
>
> The pipeline allowlist now lives in `ufw-before-output` / `ufw6-before-output`, ahead of both. It permits exactly two destinations — PostgreSQL `5432` and the CAPE API `8000`, both over `lo` — and denies everything else including DNS. `cape_api_url` is `http://10.200.0.1:8000`, the host's own WireGuard address, which Linux routes via `lo`; that is why an `-o lo` rule covers it.
>
> `network-monitor` now probes this rather than trusting it: every five minutes it attempts a connection the allowlist must refuse, because a rule that *exists* satisfied the old survey throughout the period the rule did nothing.
>
